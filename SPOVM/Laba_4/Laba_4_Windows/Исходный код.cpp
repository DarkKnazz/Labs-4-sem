...
struct Storage{					//Хранилище для данных о потоке
	int Name;					//Имя текущего потока
	int *finalNumber;			//Количество потоков в очереди
	int delay;					//Время ожидания для вывода списка
};
...
DWORD WINAPI MyThread(LPVOID lpParam){
	...
	while (true)					//Потоки работают бесконечно
	{
		...
		if (storage.Name == *(storage.finalNumber))				//Если мы последний поток,
			cout << "Name " << storage.Name << " " << endl;		//Делаем перенос строки
		else
			cout << "Name " << storage.Name << " ";				//Или же просто выводим имя потока
...
int main(){
	...
	cout << "Работа с потоками" << endl;
	cout << "Программа организует синхронизированный вывод потоков на экран" << endl;
	cout << "Также есть функция добавления и удаления потоков" << endl;
	_getch();
	system("cls");
	int a, b;
	cout << "Введите задержку между выводом потоков" << endl;
	cin >> a;
	cout << "Введите задержку между созданием новых потоков" << endl;
	cin >> b;
	CreateSemaphore(NULL, 1, 1, L"sem");
